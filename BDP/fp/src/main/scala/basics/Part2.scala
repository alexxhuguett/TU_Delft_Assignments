...
object Part2 {
  ...
  def drunkWords(xs: List[String]): List[String] = xs match {
    case Nil => Nil
    case x :: t => drunkWords(t) ::: List(x.reverse)
  }
  ...
  def myFilter[A](xs: List[A], f: A => Boolean): List[A] = {
    def helper(ys: List[A], even: Boolean): List[A] = (ys, even) match {
      case (Nil, _) => Nil
      case (h :: t, true) if (f(h)) => h :: helper(t, false)
      case (h :: t, true) if (!f(h)) => helper(t, true)
      case (h :: t, false) if (f(h)) => helper(t, true)
      case (h :: t, false) => helper(t, false)
    }
    helper(xs, true)
  }
  // END OF PART 2
