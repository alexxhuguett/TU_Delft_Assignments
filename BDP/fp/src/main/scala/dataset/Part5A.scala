...
object Part5A {
   ...
   */
  def noAge(xs: List[Array[String]]): Int = xs.count(row => row(4).trim.isEmpty)
  ...
  def highestCheckouts(xs: List[Array[String]]): Tuple3[Int, String, String] = {
    val checkouts = xs.map(x => x(2).toInt)
    val max = checkouts.max
    val patron = xs.filter(x => x(2).toInt == max)
    return (patron(0)(0).toInt, patron(0)(4), patron(0)(8))
  }
  ...
  def missing_email(xs: List[Array[String]]): Map[String, Int] = {
    xs.filter(row => row(8).toLowerCase.contains("email") && row(9).trim.equalsIgnoreCase("false"))
      .map(row => row(1))
      .groupBy(identity)
      .mapValues(_.size)
  }
