...
object Part4 {
  ...
  def reverseUsingFold[A](nrs: List[A]): List[A] =
    nrs.foldLeft(List.empty[A])((x, t) => t :: x)
  ...
  def secondLargest(xs: List[Int]): Option[Int] = {
    xs.distinct.sorted.reverse match {
    case _ :: second :: _ => Some(second) // list has at least 2 distinct elements
    case _ => None                        // otherwise no second largest
    }
  }
  ...
  def distinctAPI[A](xs: List[A]): List[A] = {
    xs.foldLeft((List.empty[A], Set.empty[A])) { case ((accList, seen), x) =>
      if (seen.contains(x)) (accList, seen)
      else (accList :+ x, seen + x)
    }._1
  }
  ...
  def specialPartition(xs: List[List[Int]]): Boolean =
    xs.flatten.filter(_ > 0).sum + xs.flatten.filter(_ < 0).map(_ + 1).sum == 0
  ...
  def determineGoodPlayers(scrabblePlayers: List[(String, List[(String, Int)])]): List[String] =
    scrabblePlayers
      .filter(p => p._2.forall(w => w._1.length > 3) && p._2.nonEmpty)
      .map(_._1)
  ...
  def avgOver7(scrabblePlayers: List[(String, List[(String, Int)])]): List[(String, Double)] =
    scrabblePlayers
      .filter(p => p._2.forall(w => w._1.length > 3) && p._2.nonEmpty)
      .map(x => (x._1, x._2.map(words => words._2).sum.toDouble / x._2.size))
      .filter(x => x._2 > 7)
  ...
  def goodSportStudents(studentResults: List[(String, String, Option[Int])], sportStudents: List[String]): List[String] = {
    val allNames = studentResults.map(_._1).distinct
    allNames.filter { name =>
      sportStudents.contains(name) &&
        studentResults.filter(_._1 == name).forall {
          case (_, _, Some(grade)) if grade >= 6 => true
          case _ => false
        }
    }.reverse
  }
  ...
  def avgGradeGoodSportStudents(studentResults: List[(String, String, Option[Int])], sportStudents: List[String]): List[(String, Double)] = {
    ...
    goodStudents.map { name =>
      val grades = studentResults.filter(_._1 == name).flatMap(_._3)
